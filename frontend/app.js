...
function preloadMedia() {
  ...
  sounds.forEach(sound => {
    ...
    audio.src = sound;
    audio.preload = 'auto';
    audio.load().catch(e => console.log('Sound preload failed'));
  });
...
function startApp() {
  ...
  function showManualShare(text) {
    showTemporaryMessage(language === 'RU' ? `📤 Поделитесь вручную: ${text}` : `📤 Share manually: ${text}`);
  }
  ...
  function showTriggerMessage(type) {
    const messages = {
      'NIGHT_TRIGGER': {
        EN: ['IT LIKES THE DARK', 'NIGHT BELONGS TO IT', 'DARKNESS IS ITS HOME', 'IT SEES BETTER IN THE DARK', 'THE NIGHT HIDES ITS MOVEMENTS'],
        RU: ['ОНО ЛЮБИТ ТЕМНОТУ', 'НОЧЬ ПРИНАДЛЕЖИТ ЕМУ', 'ТЬМА - ЕГО ДОМ', 'В ТЕМНОТЕ ОНО ВИДИТ ЛУЧШЕ', 'НОЧЬ СКРЫВАЕТ ЕГО ДВИЖЕНИЯ']
      },
      'QUICK_RETURN': {
        EN: ['YOU CAME BACK', 'IT MISSED YOU', 'RUNNING CHANGES NOTHING', 'YOU CANNOT ESCAPE', 'IT KNEW YOU WOULD RETURN'],
        RU: ['ТЫ ВЕРНУЛСЯ', 'ОНО СКУЧАЛО', 'БЕГСТВО НИЧЕГО НЕ МЕНЯЕТ', 'ТЕБЕ НЕ УЙТИ', 'ОНО ЗНАЛО, ЧТО ТЫ ВЕРНЕШЬСЯ']
      },
      'INACTIVITY': {
        EN: ['I NOTICED YOUR SILENCE', 'STILL THERE?', 'IT WATCHES YOU SLEEP', 'YOUR QUIET DOES NOT HIDE YOU', 'IT WAITS FOR YOUR RETURN'],
        RU: ['Я ЗАМЕТИЛ ТВОЕ МОЛЧАНИЕ', 'ЕЩЕ ЗДЕСЬ?', 'ОНО СЛЕДИТ ЗА ТВОИМ СНОМ', 'ТИШИНА ТЕБЯ НЕ СПАСЕТ', 'ОНО ЖДЕТ ТВОЕГО ВОЗВРАЩЕНИЯ']
      }
    };
...
ПРОДОЛЖАЯ ИСПОЛЬЗОВАНИЕ, ВЫ ПОДТВЕРЖДАЕТЕ: ОТСЧЁТ НЕ НАЧАЛСЯ - ВАМ ПРОСТО СКАЗАЛИ, СКОЛЬКО ОСТАЛОСЬ.`;
  ...
  function generateWeightedTime() {
    ...
    if (random < 0.6) days = 20 + Math.floor(Math.random() * 15);
    else if (random < 0.7) days = 1 + Math.floor(Math.random() * 9);
    else if (random < 0.9) days = (50 + Math.floor(Math.random() * 50)) * 365;
    else days = 1;
  ...
  function showGrowingNumbers() {
    ...
    numbers.forEach((num, index) => {
      html += `<div class="timer-unit number-animation" style="animation-delay: ${index * 0.1}s">${num}</div><div class="timer-label">${labels[index]}</div>`;
    });
  ...
  function updateTimerDisplay() {
    ...
    if (Math.random() < 0.03) playRandomSound();
    if (Math.random() < 0.02) showImageFlash();
    if (Math.random() < 0.05) checkForTriggers();
    if (Math.random() < 0.05) triggerHorrorEffect();
  ...
  function showRandomPhrase() {
    const phrases = language === 'RU' 
      ? ['ОН БЛИЗКО', 'ТЫ ЭТО ЧУВСТВУЕШЬ', 'НЕ СМОТРИ НАЗАД', 'ОНО ВИДИТ ТЕБЯ', 'ВРЕМЯ ИДЁТ', 'ТЫ НЕ ОДИН', 'БЕГИ', 'ПОЗДНО', 'ОНО ЗДЕСЬ']
      : ['IT IS CLOSE', 'YOU CAN FEEL IT', 'DONT LOOK BACK', 'IT SEES YOU', 'TIME IS RUNNING', 'YOU ARE NOT ALONE', 'RUN', 'TOO LATE', 'IT IS HERE'];
    ...
    setTimeout(() => phraseEl.remove(), 5000);
  }

  function showFinalScreen() {
    clearInterval(timerInterval);
    showScreen(`<div class="final-screen red">${language === 'RU' ? 'ОНО ИДЁТ ЗА ТОБОЙ' : 'IT IS COMING FOR YOU'}</div>`);
  }
